...
### Generate a Video From a Code Repository

This checkout includes a small wrapper that scans a source-code repository, turns its structure into an LTX-2 prompt using the guidance above, and then launches one of the existing video pipelines.

Preview the generated prompt and command without loading models:

```bash
python scripts/repo_to_video.py \
  --repo /path/to/code-repo \
  --style cartoon \
  --subject "a kid astronaut discovering a candy planet" \
  --dry-run \
  --print-command \
  --prompt-output outputs/repo-prompt.txt
```

Use `--style cinematic` for the default realistic repo-visualization look, or `--style cartoon` / `--cartoon` for bright animated cartoon videos. Add `--subject "..."` to choose the cartoon story; without it, the script uses a cheerful inventor scene.

Run the fast distilled pipeline:
...
python scripts/repo_to_video.py \
  ...
  --subject "a kid astronaut discovering a candy planet" \
  --pipeline distilled \
  --distilled-checkpoint-path /path/to/ltx-2.3-22b-distilled-1.1.safetensors \
  --spatial-upsampler-path /path/to/ltx-2.3-spatial-upscaler-x2-1.1.safetensors \
  --gemma-root /path/to/gemma \
  --output-path outputs/repo-video.mp4 \
  --height 768 \
  --width 1280 \
  --num-frames 97 \
  --frame-rate 24
...
Video generation requires an NVIDIA CUDA GPU and a CUDA-enabled PyTorch install. The included pipeline scripts do not support local Mac CPU/MPS generation; on macOS you can still use `--dry-run` to build prompts, then run the generated command on a CUDA machine.

If you are using the Gemma text encoder from Hugging Face, you must accept the gated model license before cloning it:

1. Open <https://huggingface.co/google/gemma-3-12b-it-qat-q4_0-unquantized>.
2. Log in and click **Acknowledge license**.
3. Create or use a Hugging Face read token from <https://huggingface.co/settings/tokens>.
...
Then clone the Gemma repository and use it as `--gemma-root`:
...
git lfs install
git -C gemma-3-12b-it-qat-q4_0-unquantized lfs pull
git -C gemma-3-12b-it-qat-q4_0-unquantized lfs checkout
```

The Gemma folder should be many gigabytes after the LFS pull completes.

When using multi-line shell commands, keep `\` as the final character on continued lines. Do not add spaces after it, and do not put it on the final line.

Advanced pipeline options can be passed through after the wrapper arguments, for example `--quantization fp8-cast` or `--offload cpu`.
